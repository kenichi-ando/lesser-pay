...
4. **承認 / 編集 / 訂正依頼** (親) — ヘッダー右上の**ユーザ名チップ**をタップ → 必要に応じて「ログインユーザーを切り替え」から保護者でログイン → `申請中` の課題に「✓承認」「編集/訂正依頼」。
...
6. **ポイント消費** (親) — 保護者モードで「💸 ポイント消費」→ 履歴に `-XXX` で記録。子ども端末に通知。
